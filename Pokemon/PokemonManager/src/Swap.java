...
public class Swap {
	...
	public static Swap execute(Pokemon pokemon1, Pokemon pokemon2) {
		...
		if (!pokemon1.isSwapAllow()) {
			System.err.println(pokemon1+" is not swapable.");
			return null;
		}
		if (!pokemon2.isSwapAllow()) {
			System.err.println(pokemon2+" is not swapable.");
			return null;
		}
		if (pokemon1.getType() == pokemon2.getType()) {
			System.err.println(pokemon1+" and "+pokemon2+" are of the same type.");
			return null;
		}
		if (pokemon1.getTrainer() == pokemon2.getTrainer()) {
			System.err.println(pokemon1+" and "+pokemon2+" have the same trainer.");
			return null;
		}
